...
export class AppbdService {
  ...
  viaje: string = "CREATE TABLE IF NOT EXISTS viaje(id_viaje INTEGER PRIMARY KEY autoincrement, fechahora TEXT NOT NULL, asientos_disp INTEGER NOT NULL, pasajeros INTEGER NOT NULL, costo INTEGER NOT NULL, status VARCHAR(30) NOT NULL, inicio BLOB, destino BLOB, id_usuario INTEGER NOT NULL, id_sede INTEGER NOT NULL, FOREIGN KEY(id_usuario)REFERENCES usuario, FOREIGN KEY(id_sede)REFERENCES sede);";
  ...
  registroSede1: string = "INSERT or IGNORE INTO sede(id_sede,sede) VALUES (1,'Huecheraba');";
  registroSede2: string = "INSERT or IGNORE INTO sede(id_sede,sede) VALUES (2,'Colina');";
  registroSede3: string = "INSERT or IGNORE INTO sede(id_sede,sede) VALUES (3,'Conchalí');";
  ...
  buscarViaje(id_sede) {
    let data = [id_sede];
    return this.database.executeSql('SELECT V.id_viaje as viaje,date(fechahora) as fecha,time(fechahora) as hora,asientos_disp,pasajeros,costo,inicio,destino,status,rol_usuario,U.id_usuario,id_sede,U.foto as foto,nombre,apellido,correo FROM viaje V JOIN detalle_viaje D ON V.id_viaje = D.id_viaje JOIN usuario U on D.id_usuario = U.id_usuario WHERE id_sede = ? AND rol_usuario = 1;', data).then(res => {
  ...
  async crearViaje(hora, asientos_disp, costo, inicio, destino, id_usuario, id_sede) {
    ...
    await this.database.executeSql('SELECT id_usuario FROM detalle_viaje WHERE id_usuario = ? ;', userid).then(res => {
      if (res.rows.length > 0) {
        ...
        let data = [hora, asientos_disp, pasajeros, costo, inicio, destino, status, id_usuario, id_sede];
        this.database.executeSql('INSERT INTO viaje(fechahora,asientos_disp,pasajeros,costo,inicio,destino,status,id_usuario,id_sede) VALUES (?,?,?,?,?,?,?,?,?);', data);
